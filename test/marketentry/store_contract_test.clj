...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X SARL" :jurisdiction "LUX"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "LUX" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (true? (:etablissement-fixed-place-not-domiciliation-only? (:engagement m))))
    (is (true? (:etablissement-fixed-place-not-domiciliation-only? (:engagement d))))
