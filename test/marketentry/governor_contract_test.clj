  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kaell Digital Services SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kaell Digital Services SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest establishment-conditions-unmet-is-held-and-unoverridable
  (testing "domiciliation-only engagement (Art. 5 final alinéa carve-out) -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
          _ (assess! actor "t5pre" "eng-5")
          _ (draft! actor "t5pre" "eng-5")
          res (exec-op actor "t5" {:op :filing/submit :subject "eng-5"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:establishment-conditions-unmet} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest restricted-procedure-threshold-mismatch-is-held-and-unoverridable
  (testing "claimed light-touch-procedure eligibility doesn't match the independently recomputed Art. 151 (as amended 2024) ceiling -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:restricted-procedure-threshold-mismatch} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
