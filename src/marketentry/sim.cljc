...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (LUX, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up restricted-procedure-threshold-mismatch) ==")
    ...
    (println "== filing/submit eng-4 (restricted-procedure-threshold-mismatch -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up establishment-conditions-unmet -- domiciliation only) ==")
    ...
    (println "== filing/submit eng-5 (establishment-conditions-unmet -> HARD hold, FLAGSHIP) ==")
